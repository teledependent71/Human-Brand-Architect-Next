...
import postsPageInitialPaths663e6Resource from '../../resources/posts-page-initial-paths-663e6'
import postsPageInitialProps9cd30Resource from '../../resources/posts-page-initial-props-9cd30'
  ...
  return (
    <>
      <div className="posts11-container">
        <Head>
          <title>Posts1 - Human Brand Architect</title>
          <meta property="og:title" content="Posts1 - Human Brand Architect" />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await postsPageInitialPaths663e6Resource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await postsPageInitialProps9cd30Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
