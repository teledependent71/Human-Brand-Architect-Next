...
import authorsPageInitialPathsEa799Resource from '../../resources/authors-page-initial-paths-ea799'
import authorsPageInitialProps415d6Resource from '../../resources/authors-page-initial-props-415d6'
  ...
  return (
    <>
      <div className="authors11-container">
        <Head>
          <title>Authors1 - Human Brand Architect</title>
          <meta
            property="og:title"
            content="Authors1 - Human Brand Architect"
          />
        </Head>
        <DataProvider
...
export async function getStaticPaths() {
  try {
    const response = await authorsPageInitialPathsEa799Resource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await authorsPageInitialProps415d6Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
